...
def signup():
    if request.method=="POST":
        ...
        try:
            ...
            db.session.rollback()
            flash('Username or email already exists! Try with a new One') 
...
def login():
    if request.method=="POST":
        ...
        if user and user.password==password:
            ...
            if not user:
                error_message="No user found wiuth the provided username or email"
...
def admin_dashboard(curr_login_id):
    if request.method=="GET":
        if 'user_id' in session and session['user_id']==curr_login_id:
            user=User.query.get(curr_login_id)
            if not user.admin:
                flash("You are not authorized to access this dashbaord")
        ...
        flash("please login into your dashbaord")
        return redirect(url_for('logout'))
...
@app.route('/admin/<int:curr_login_id>/create_product/<int:product_id>',methods=["GET","POST"])
